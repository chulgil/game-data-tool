...
class CSharpManager:
    ...
    def __init__(self, branch: str, tag: str, commit_id, save_dir: Path):
        ...
        self.PATH_FOR_ENTITY = self.PATH_FOR_SAVE.joinpath('AutoGenerated_PlaningEntity.cs')
        self.PATH_FOR_ENUM = self.PATH_FOR_SAVE.joinpath('AutoGenerated_PlaningEnum.cs')
        self.PATH_FOR_S_PROTOCOL = self.PATH_FOR_SAVE.joinpath('AutoGenerated_ServerProtocol.cs')
        self.PATH_FOR_S_ENTITY = self.PATH_FOR_SAVE.joinpath('AutoGenerated_ServerEntity.cs')
        self.PATH_FOR_S_ENUM = self.PATH_FOR_SAVE.joinpath('AutoGenerated_ServerEnum.cs')
        self.PATH_FOR_S_PROXY = self.PATH_FOR_SAVE.joinpath('AutoGenerated_ServerProxy.cs')
        self.TAB = '    '
        # Config 파일 설정
...
using Server;'''
    ...
    def save_server_enum(self, enum_info: dict):
        ...
        _script = ''
        for _enum_name, enum_data in enum_info.items():
            _dlist = [f'{self._comment(1)} {x.strip()}</br>' for x in enum_data['desc'] if x.strip() != '']
            _cls_desc = '\n'.join(_dlist)
    ...
    def save_server_entity(self, info: dict):
        ...
        _name_space = 'AutoGenerated.Server.Protocol'
        _script = ''
        for _enum_name, enum_data in info.items():
            _dlist = [f'{self._comment(1)} {x.strip()}</br>' for x in enum_data['desc'] if x.strip() != '']
            _cls_desc = '\n'.join(_dlist)
            _script = _script + f'\n{self._comment(1)} <summary>\n{_cls_desc}\n{self._comment(1)}</summary>'
            # _script = _script + self._convert_enum(_enum_name, enum_data['items'], 1)
    ...
    def _convet_entity(self, table_name: str, rows: list) -> str:
        ...
        try:
            start = self.TAB
            newline = '\n' + start
            script = script + newline + f'public class {table_name} : TABLE_DATA'
            ...
            for row in rows[1:]:
                ...
                script = script + self.TAB + f'public {d_type} {field} ' + '{ get; set; } '
                script = script + ('// ' + desc + newline if desc != '' else newline)
...
{{
    {3}
}}'''.format(self.TAG, self.COMMIT_URL, self.COMMIT_ID, script)
...
{{
    {5}
}}'''.format(self.TAG, self.COMMIT_URL, self.COMMIT_ID, using, namespace, script)
    ...
    def _get_script_protocol(self, cls_name: str, cls_desc: list, req: dict, res: dict):
        ...
        _dlist = [f'{self._comment(1)} {x.strip()}<br/>' for x in _cls_desc if x.strip() != '']
        _dlist.insert(0, '')
        _cls_desc = '\n'.join(_dlist)
    ...
    def _get_script_proxy(self, cls_name: str, cls_desc: list, req: dict, res: dict):
        ...
        # /// 프로토콜 정보 : 요청[101] 응답[102]
        _cls_desc.append(f' 프로토콜 코드 [ 요청 : {_req_id}, 응답 : {_res_id} ]')
        _dlist = [f'{self._comment(2)} {x.strip()}<br/>' for x in _cls_desc if x.strip() != '']
        _dlist.insert(0, '')
        _cls_desc = '\n'.join(_dlist)
    ...
    def _get_script_server_entity(self, cls_name: str, cls_desc: list, req: dict, res: dict):
        # /// 프로토콜 설명입니다.
        # /// 프로토콜 정보 : 요청[101] 응답[102]
        _req_id = next(iter(req))
        _res_id = next(iter(res))
